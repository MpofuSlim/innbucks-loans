 ...
 * Mirrors the ticketing platform's WhatsApp client and consumes the same
 * {@code whatsapp.*} config ({@code WHATSAPP_GATEWAY_URL} /
 * {@code WHATSAPP_API_KEY}).
 *
 * <p>Any rejection or connectivity failure is surfaced as a
 * {@link NotificationDeliveryException} so the caller can decide whether to fail
 * the operation (e.g. a password reset) or treat it as best-effort.
 *
...
public class WhatsAppNotificationClient {
    ...
    public void sendCustomNotification(String to, String notification) {
        ...
        if (notification == null || notification.isBlank()) {
            throw new NotificationDeliveryException("WhatsApp message is blank");
        }
        ...
        try {
            ...
            log.warn("WhatsApp gateway unreachable to={} error={}", to, ex.getMessage());
            throw new NotificationDeliveryException(
